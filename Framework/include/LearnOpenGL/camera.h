...
const float SpeedMultiplier = 4.5f;
...
public:
	...
	Camera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f),
		...
		Zoom(ZOOM),
		m_fNearClipPlane(nearClipPlane),
		m_fFarClipPlane(farClipPlane)
	{
		Position = position;
		WorldUp = up;
		Yaw = yaw;
	...
	glm::mat4 GetProjectionMatrix(float a_screenWidth, float a_screenHeight)
	{
	...
	void ProcessKeyboard(Camera_Movement movement, float deltaTime)
	{
		...
		if (movement == MOVE_FASTER)
		{
			MovementSpeed = SPEED * MovementSpeedMultiplier;
		}
		else
		{
			MovementSpeed = SPEED;
